...
return baseclass.extend({
	...
	renderLanguageSelect: function () {
		const setLanguage = async function (lang) {
			await callUciSet('luci', 'main', { ['lang']: lang });
			await callUciCommit('luci');
			window.location.reload();
		};

		var container = document.querySelector('#language-select');
		var englishSelect = E('button', {
			class: 'btn link',
			click: () => setLanguage('en'),
		}, 'English');
		var japaneseSelect = E('button', {
			class: 'btn link',
			click: () => setLanguage('ja'),
		}, '日本語');

		container.appendChild(englishSelect);
		container.appendChild(E('span', {}, '&nbsp;|&nbsp;'));
		container.appendChild(japaneseSelect);
	},
	...
	renderMainMenu: function (tree, url, level) {
		...
		for (var i = 0; i < children.length; i++) {
			...
			// Because we auto-login, no point offering logout.
			if (document.getElementById('password-warning') && linkurl === '/cgi-bin/luci/admin/logout') {
				continue;
			}
